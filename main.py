...
#    kopen en verkopen van producten de data en prijzen opslaan per data- Lets go
#    check if products are expired--------------------------------------- done
#    View Inventory') --------------------------------------------------- done
#    Buy items')   ------------------------------------------------------ done   
#    sell items')    ---------------------------------------------------- done
#    view reports') 1. buy report 2. sell report+ expired products------- done
#    advance date') ----------------------------------------------------- Done
#    view  itemlist)----------------------------------------------------- Done
#    add items to item list) -------------------------------------------- done
#    remove items from item list) --------------------------------------- done
#    work with quantity--------------------------------------------------
#    inventory also works for past dates---------------------------------
#    use rich ----------------------------------------------------------- done
#    use matplotlib (numpy)---------------------------------------------- started

def main():
    pass

    current_directory = os.getcwd()
    files_map = current_directory+'\\files'
    ...
    def create_files():
        ...
        with open(files_map+'\\itemlist.csv', 'w', newline='') as file:
            writer = csv.writer(file)
            writer.writerow(['product','buy_price','sell_price',])
        with open(files_map+'\\buy.csv', 'w', newline='')as file:
            writer = csv.writer(file)
            writer.writerow(['bought_id','product','buy_date','buy_price','quantity','expiration_date','status'])
        with open(files_map+'\\id.txt', 'w') as file:
            file.write('1')
        with open(files_map+'\\sold.csv', 'w', newline='')as file:
            writer = csv.writer(file)
            writer.writerow(['sell_id','bought_id','product','sell_date','buy_price','sell_price','quantity','status'])
        with open(current_directory+'\\readme.txt', 'w') as file:
    ...
    def is_expired():
        date = get_date()
        with open(files_map+'\\'+'buy.csv', 'r') as read_obj:
            ...
            for row in reader:
                if date >= row['expiration_date']:
                   ...
                   status = 'expired'
                   new_row = sell_id, bought_id, product, sell_date, buy_price, sell_price , quantity, status
    ...
    def add_to_csv(file_name, line_to_add):
        with open(files_map+'\\'+file_name, 'a+', newline='') as write_obj:
        ...
        with open(files_map+'\\'+file_name, 'r') as read_obj:
            ...
            for row in reader:
                if to_remove != row[colomn]:
                    ...
                    (exit)
        with open(files_map+'\\'+file_name, 'w', newline= '') as new_obj:
    ...
    def get_date():
        with open(files_map+'\\today.txt','r') as read:
            for line in read:
                return line
    
    #function to make a id number
    def get_id():
        with open(files_map+'\\id.txt','r') as read:
            for line in read:
                if line:
    ...
    def get_sold(date):
        data = []
        with open(files_map+'\\'+'sold.csv', 'r') as read:
    ...
    def get_price(product, buy_or_sell):
        with open(files_map+'\\itemlist.csv') as csv_file:
    ...
    def view_inventory():
        ...
        with open(files_map+'\\buy.csv') as csv_file:
            csv_reader = csv.DictReader(csv_file)
        ...
        with open(files_map+'\\itemlist.csv') as itemlist:
            itemreader = csv.DictReader(itemlist)
            with open(files_map+'\\buy.csv') as buy:
    ...
    def advance_time():
        ...
        new_date = (datetime.datetime.today() + datetime.timedelta(days=days_to_skip)).strftime('%Y-%m-%d')
        confirm = ''
    ...
    def remove_item():
        products = ''
        with open(files_map+'\\'+'itemlist.csv', 'r') as read_obj:
            reader = csv.DictReader(read_obj)  
            for row in reader:
    ...
    #test parser
    #test_parser = subparser.add_parser('test', help= 'testing of functions' )
    #test_parser.set_defaults(func=remove_from_csv('buy.csv','1'))
